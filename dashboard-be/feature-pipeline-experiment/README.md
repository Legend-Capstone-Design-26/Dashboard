...
최종 실행 경로는 `dashboard-be/feature-pipeline-experiment/` 이다. `dashboard-be/experiment/` 는
기존 모듈로 보존하지만 이 실험의 실행/검증 범위 밖이므로 수정하거나 결과 비교에 사용하지 않는다.
...
결과는 [REPORT.md](REPORT.md) 에 표로 정리된다. 현재 커밋된 `REPORT.md`,
`artifacts/results.csv`, `artifacts/split.json` 은 최종 수정 전 생성된 pre-fix 산출물로
보존되어 있으며, 새 실행 결과는 별도 output 경로에 생성해서 비교한다. 소요 시간은 약 20분이며 대부분 A2(UMAP)에서 쓴다.
