...
METRIC_ORDER = ["ari", "nmi", "ami", "macro_f1", "hungarian_accuracy", "silhouette", "davies_bouldin"]
...
def ensure_result_columns(frame):
    prepared = frame.copy()
    for column in OPTIONAL_RESULT_COLUMNS:
        if column not in prepared.columns:
            prepared[column] = 0.0 if column == "umap_fallback_ratio" else float("nan")
    return prepared


def aggregate(frame):
    prepared = ensure_result_columns(frame)
    grouped = prepared.groupby(["feature_subset", "pipeline"])
    summary = grouped.agg(
        ...
        acc_mean=("hungarian_accuracy", "mean"), acc_std=("hungarian_accuracy", "std"),
        ...
        selected_n_neighbors_mean=("selected_n_neighbors", "mean"),
        selected_min_cluster_size_mean=("selected_min_cluster_size", "mean"),
        selected_min_samples_mean=("selected_min_samples", "mean"),
        umap_fallback_mean=("umap_fallback_ratio", "mean"),
        selected_latent_dim_mean=("selected_latent_dim", "mean"),
    ).reset_index()
    summary[["ari_std", "nmi_std", "ami_std", "f1_std", "acc_std"]] = summary[
        ["ari_std", "nmi_std", "ami_std", "f1_std", "acc_std"]
    ].fillna(0.0)
...
def main_table(summary):
    lines = [
        "| feature | pipeline | seeds | runtime(s) | 선택 params | ARI mean | ARI std | AMI mean | AMI std | "
        "NMI mean | Macro-F1 mean | Macro-F1 std | Hungarian Acc | Silhouette | DB | fallback | 비고 |",
        "|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|",
    ]
    for _, row in summary.iterrows():
        lines.append(
            f"| {row['feature_subset']} | {row['pipeline']} | {int(row['seeds'])} | "
            f"{row['runtime_mean']:.1f} | {selected_params(row)} | "
            f"{row['ari_mean']:.3f} | {row['ari_std']:.3f} | "
            f"{row['ami_mean']:.3f} | {row['ami_std']:.3f} | {row['nmi_mean']:.3f} | "
            f"{row['f1_mean']:.3f} | {row['f1_std']:.3f} | "
            f"{row['acc_mean']:.3f} | {row['sil_mean']:.3f} | {row['db_mean']:.3f} | "
            f"{fallback_note(row)} | {note_for(row)} |"
        )
    return "\n".join(lines)


def selected_params(row):
    if row["pipeline"] == "A2":
        return (
            f"n={row['selected_n_neighbors_mean']:.1f}, "
            f"min_cluster={row['selected_min_cluster_size_mean']:.1f}, "
            f"min_samples={row['selected_min_samples_mean']:.1f}"
        )
    if row["pipeline"] == "A3":
        return f"latent={row['selected_latent_dim_mean']:.1f}, k={row['selected_k_mean']:.1f}"
    return f"k={row['selected_k_mean']:.1f}"


def fallback_note(row):
    if row["pipeline"] == "A2":
        return f"UMAP {row['umap_fallback_mean']:.1%}"
    return "-"
