...
function createMetricsTools({ metricsReadModel, experimentStore }) {
  ...
  function summarizeExperimentMetrics({ siteId, key, selectedExperimentKey }) {
    ...
    if (!resolvedKey) return { ok: false, reason: "missing_experiment_key", message: "요약할 실험을 찾지 못했습니다. 실험을 먼저 선택해 주세요." };

    const metrics = metricsReadModel.getExperimentMetrics({ siteId, key: resolvedKey });
    if (!metrics?.ok) return { ok: false, reason: metrics?.reason || "metrics_failed", message: "선택한 실험의 metrics를 불러오지 못했습니다." };
  ...
  return { summarizeExperimentMetrics };
}
